function [varargout] = binlabels(var, nv, fig)
...
if isfield(nv, 'rightedge')
    if strcmpi(nv.rightedge, 'delete')
        xlbs(end) = [];
    else
        xlbs(end) = append(var(end, 1), '-', nv.rightedge);
    end
end
