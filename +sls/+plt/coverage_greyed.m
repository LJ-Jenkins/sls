...
%   'axis' - (default, gca) specify different axis object to plot onto
%
% Outputs (in order):
...
if isfield(fig, 'addtofig')
    ...
    yl = ylim(fig.axis);
    % f = double.empty;
    f = gobjects(1, length(x) - 1);
    for q = 1:length(x) - 1
        if q ~= length(x) - 1
            f(q) = fill(fig.axis, [repmat(datetime(x(q), 1, 1, 0, 0, 0), 1, 2) ...
    ...
    % uistack(f,'bottom')
    set(fig.axis, 'children', flipud(get(fig.axis, 'children')))
end
if nargout >= 1
    out = {percs, alphas};
